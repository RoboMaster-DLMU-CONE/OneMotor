...
struct RawStatusFrame {
    ...
    [[nodiscard]] std::string format() const {
#ifdef ONE_MOTOR_LINUX
        return std::format("ID: {:X}, Ang: {} RPM, ecd: {} / 8191, CRT: {} mA, "
                           "temperature: {} deg",
        ...
        oss << "ID: " << std::hex << std::uppercase << canId << std::dec
            << ", Ang: " << rpm << " RPM, ecd: " << ecd
            << " / 8191, CRT: " << current
            << " mA, temperature: " << temperature << " deg";
        return oss.str();
#endif
    };
...
struct MotorStatus {
    ...
    [[nodiscard]] std::string format() const {
        ...
        return std::format("M3508 Status:\n"
                           ...
                           output_current, temperature);
#else
        std::ostringstream oss;
        oss << "M3508 Status:\n"
            << "\t- ECD: " << ecd << " (last: " << last_ecd << ")\n"
            << "\t- Angle (single round): " << std::fixed
            << std::setprecision(2) << angle_single_round << " deg\n"
