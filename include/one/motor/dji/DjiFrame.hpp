...
struct MotorStatusPlain {
    float angle_single_round_rad{}; ///< 单圈角度 (0-2pi)
    float angular_rad_s{};          ///< 角速度 (rad/s)
    float real_current_mA{};        ///< 实际电流 (mA)
    float temperature_C{};          ///< 温度 (°C)
    float total_angle_rad{};        ///< 累计总角度 (rad)，带方向
    int32_t total_round{};          ///< 累计总圈数，带方向
    float reduced_angle_rad{};      ///< 累计减速后的总角度（rad)，带方向
    float reduced_angular_rad_s{};  ///< 累计减速后的总角度（rad/s)，带方向
};

struct MotorStatus {
    units::AngularVelocity angular{};    ///< 角速度 (度/秒)
    ...
    static MotorStatus fromPlain(const MotorStatusPlain &plain) {
        return {.angular = plain.angular_rad_s * rad / s,
                ...
                .total_angle = plain.total_angle_rad * rad,
                .reduced_angle = plain.reduced_angle_rad * rad,
                .reduced_angular = plain.reduced_angular_rad_s * rad / s};
    }
