...
namespace one::can
{
    ...
    public:
         ...
         */
        tl::expected<void, motor::Error> init(const device* device);
#endif
            ...
            requires(std::invocable<Func, CanFrame>)
        {
            ...
            return interface->register_callback<can_frame>(
                             ...
                             .map_error([&](const auto&)
                              {
                                  return motor::Error{
                                      motor::ErrorCode::CanDriverInternalError,
        ...
        template <typename Func>
        tl::expected<void, motor::Error> registerCallback(const std::set<size_t>& can_ids,
                                                          Func&& func)
            requires(std::invocable<Func, CanFrame>)
        {
            if (auto guard = ensureInitialized(); !guard)
    ...
    inline tl::expected<void, motor::Error> CanDriver::ensureInitialized() const
    {
        ...
        return tl::make_unexpected(
            motor::Error{
                motor::ErrorCode::CanDriverNotInitialized,
